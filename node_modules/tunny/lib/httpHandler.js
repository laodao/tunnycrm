...
exports.handleDynamicFile = function (req, res, action, webForm) {
    ...
    //动态请求处理
    logger.info('start process dynamic req');
    if (req.method.toUpperCase() == 'GET') {//处理get请求
        webForm.setReq(req);
        webForm.setMethod(method);
        invokeAction(req, res, action, webForm, send);
        return;
    } else if (req.method.toUpperCase() == 'POST'||req.method.toUpperCase() == 'DELETE'||req.method.toUpperCase() == 'PUT') {//处理post请求
...
//    var action = require(controllerPath);
    ...
    if(!interceptor){
        interceptor = {
            before:function(form, fn){
                logger.info('no interceptor')
                fn();
            },
            after:function(form, fn){
                fn();
            }
        };
    }
...
//    }
    ...
    render.forward = function(url){
        interceptor.after(webForm, function(){//执行after拦截器，无论after拦截器做了什么操作，或者返回什么，请求都将继续执行下去
            forward(req, res, url, webForm);
        });
        //return;
    };

    render.redirect = function(url){
        interceptor.after(webForm, function(){//执行after拦截器，无论after拦截器做了什么操作，或者返回什么，请求都将继续执行下去
            redirect(req, res, url, webForm.getMsg());
        });
    };
...
//        logger.info('start call the action method:'+fn);
 ...
 * @param url
 */
var redirect = function (req, res, url, msg) {
...
//        setCookies[setCookies.length] = ['Set-Cookie', cookies.get(key).serialize()];
        setCookies[setCookies.length] = cookies.get(key).serialize();
    }
    var clientIP = req.headers['x-forwarded-for']
        || req.connection.remoteAddress;
    //logger.info('response at '+new Date().toString()+",client ip:"+clientIP+',http status 200');
    res.writeHead(200, {'Set-Cookie':setCookies, 'Content-Type':contentType});
    res.end(retStr, "utf8");
};
